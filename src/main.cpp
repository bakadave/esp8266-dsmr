...
const Measurement measurements[] = {
   ...
   {"power/timestamp", "0-0.9.1.255", 8, false, Measurement::STRING},
   {"power/device_id", "0-0:96.1.0", 8, false, Measurement::STRING},
   {"power/consumption", "1-0.1.7.0", 7, false, Measurement::FLOAT},
   ...
   {"power/production", "1-0.2.7.0", 7, false, Measurement::FLOAT},
   ...
   {"power/phase_1/current", "1-0.31.7.0", 7, false, Measurement::FLOAT},
   {"power/phase_2/current", "1-0.51.7.0", 7, false, Measurement::FLOAT},
   {"power/phase_3/current", "1-0.71.7.0", 7, false, Measurement::FLOAT},
   {"power/phase_1/voltage", "1-0.32.7.0", 6, false, Measurement::FLOAT},
   {"power/phase_2/voltage", "1-0:52.7.0", 6, false, Measurement::FLOAT},
   {"power/phase_3/voltage", "1-0:72.7.0", 6, false, Measurement::FLOAT},
...
void handleString(String incomingString)
{
  for (int i = 0; i < sizeof(measurements) / sizeof(measurements[0]); i++)
  {
    ...
    int start = measurement.key.length();
    int end = start + measurement.len;

    if (incomingString.indexOf(obisKey) > -1)
    {
      // found
      String value = incomingString.substring(start, end);
